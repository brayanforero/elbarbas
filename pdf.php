...
############################################# FUNCION PARA MOSTRAR EL FOOTER ##########################################
  ...
  $this->SetFont('Arial','B',6);
        //Número de página
  $this->Cell(190,4,'Sistema de Control de Ventas y Facturacion','T',0,'L');
  $this->AliasNbPages();
...
############################################### FUNCION LISTAR LOGS DE ACCESO DE USUARIOS ######################################################
	  ...
	  function TablaListarLogs()
   {
	...
	$this->Cell(30,5,'',0,0,'');
	$this->CellFitSpace(42,5,DNI.' GERENTE :',0,0,'');
...
############################################### FUNCION LISTAR CAJAS DE VENTAS ######################################################
	  ...
	  function TablaListarCajas()
   {
	...
	$this->SetFillColor(2,157,116);
	$this->Cell(10,5,'',0,0,'');
	$this->Cell(35,5,DNI.' GERENTE: ',0,0,'');
...
############################################### FUNCION LISTAR PROVEEDORES ######################################################
	  function TablaListarProveedores()
   {
    ...
    $this->SetFillColor(249, 187, 31); // establece el color del fondo de la celda (en este caso es NARANJA)
	$this->Cell(10,8,'N°',1,0,'C', True);
	$this->Cell(28,8,IDENTIFICACION2,1,0,'C', True);
	$this->Cell(70,8,'NOMBRES',1,0,'C', True);
...
################################################# BLOQUE N° 2 ###################################################	
	...
	$this->SetFont('courier','B',7);
    $this->SetXY(147, 34);
    $this->Cell(90, 5, IDENTIFICACION.' :', 0 , 0);
	$this->SetFont('courier','',7);
    $this->SetXY(156, 34);
...
################################################# BLOQUE N° 3 ###################################################	
	...
	$this->SetFont('courier','B',7);
    $this->SetXY(100, 54);
    $this->Cell(70, 5, IDENTIFICACION.' :', 0 , 0);
	$this->SetFont('courier','',7);
    $this->SetXY(108, 54);
...
################################################# BLOQUE N° 3 ###################################################	
	...
	$this->SetFont('courier','B',7);
    $this->SetXY(100, 56);
    $this->Cell(70, 5, IDENTIFICACION2.' :', 0 , 0);
	$this->SetFont('courier','',7);
    $this->SetXY(108, 56);
...
################################################# BLOQUE N° 5 DE TOTALES ###################################################	
	...
	$this->SetFont('courier','B',9);
    $this->SetXY(124, 256);
    $this->Cell(20, 5, 'SUBTOTAL '.IMPUESTO.' 0% :', 0 , 0);
    $this->SetXY(167, 256);
	$this->SetFont('courier','',9);
	...
	$this->SetFont('courier','B',9);
    $this->SetXY(124, 260);
    $this->Cell(20, 5, IMPUESTO.' '.$co[0]["ivac"].'% :', 0 , 0);
    $this->SetXY(167, 260);
	$this->SetFont('courier','',9);
...
############################################### FUNCION LISTAR COMPRAS GENERALES ######################################################
   ...
   {	
	...
	$this->CellFitSpace(30,8,'SUBTOTAL',1,0,'C', True);
	$this->CellFitSpace(30,8,'SUBTOTAL '.IMPUESTO.' 0%',1,0,'C', True);
	$this->CellFitSpace(15,8,IMPUESTO,1,0,'C', True);
	$this->CellFitSpace(25,8,'TOTAL '.IMPUESTO,1,0,'C', True);
...
################################################# BLOQUE N° 3 ###################################################	
	...
	$this->SetFont('courier','B',7);
    $this->SetXY(15, 56);
    $this->Cell(20, 5, 'NOMBRE CLIENTE:', 0 , 0);
	$this->SetFont('courier','',7);
    $this->SetXY(38, 56);
    ...
    if($reg[$i]["ivaproducto"]=="SI"){
      $preiva=$reg[$i]["precioventa"]+($reg[$i]["precioventa"]*($ve[0]['ivave']/100));
    }else{
      $preiva=$reg[$i]["precioventa"];
    }
	$this->CellFitSpace(25,4,utf8_decode(number_format($preiva, 2, '.', ',')),0,0,'C');
	$this->CellFitSpace(23,4,utf8_decode($reg[$i]["cantventa"]),0,0,'C');
  if($reg[$i]["ivaproducto"]=="SI"){
      $preimport=$reg[$i]["importe"]+($reg[$i]["importe"]*($ve[0]['ivave']/100));
    }else{
      $preimport=$reg[$i]["importe"];
    }
	$this->CellFitSpace(25,4,utf8_decode(number_format($preimport, 2, '.', ',')),0,0,'C');
    $this->Ln();
  }
...
################################################# BLOQUE N° 5 DE TOTALES ###################################################	
  ...
  $this->SetFont('courier','',9);
	...
	$this->SetFont('courier','B',9);
    $this->SetXY(124, 258);
    $this->Cell(20, 5, IMPUESTO.' '.$ve[0]["ivave"].'% :', 0 , 0);
    $this->SetXY(167, 258);
	$this->SetFont('courier','',9);
...
############################################### FUNCION TICKET DE VENTAS DE PRODUCTOS ######################################################
	  function TablaTicketProductos()
   {
	...
	$this->SetXY(4, 15);
	$this->CellFitSpace(65,3,IDENTIFICACION.":".utf8_decode($con[0]['rifempresa']),0,1,'C');
	$this->SetXY(4, 17);
    ...
    $this->SetTextColor(3,3,3);  // Establece el color del texto (en este caso es negro)
    $this->CellFitSpace(5,3,utf8_decode("CAN"),0,0,'C');
    $this->CellFitSpace(45,3,utf8_decode("PRODUCTO"),0,0,'L');
    $this->CellFitSpace(17,3,utf8_decode("PREC"),0,0,'R');
    $this->Ln();
  ...
  $subtot=$reg[$i]["precioventa"]+($reg[$i]["precioventa"]*($ve[0]["ivave"]/100));
	 $this->CellFitSpace(17,3,utf8_decode(number_format($subtot, 2, '.', ',')),0,0,'R');
     $this->Ln();
     $subtot=0;	 
   }
	...
	$this->SetFillColor(2,157,116);
	$this->SetX(4);
	$this->CellFitSpace(60,3,"SUBTOTAL :".utf8_decode(number_format($ve[0]["subtotalivasive"], 2, '.', ',')),0,1,'L');
	$this->SetX(4);
	/*$this->CellFitSpace(60,3,"SUBTOTAL ".IMPUESTO." 0%: ".utf8_decode(number_format($ve[0]["subtotalivanove"], 2, '.', ',')),0,1,'L');
	$this->SetX(4);*/
	$this->CellFitSpace(60,3,IMPUESTO." ".$ve[0]["ivave"].'% : '.utf8_decode(number_format($ve[0]["totalivave"], 2, '.', ',')),0,1,'L');
	$this->SetX(4);
	$this->SetFont('courier','',6);
	$this->CellFitSpace(60,3,"DESCUENTO ".$ve[0]["descuentove"].'% : '.utf8_decode(number_format($ve[0]["totaldescuentove"], 2, '.', ',')),0,1,'L');
	$this->SetX(4);
	
	$this->CellFitSpace(60,3,"TOTAL: ".utf8_decode(number_format($ve[0]["totalpago"], 2, '.', ',')),0,1,'L');
  $this->Ln();
  $this->SetFont('courier','B',6);
	$this->SetX(4);
	$this->CellFitSpace(60,3,"TIPO PAGO: ".utf8_decode($ve[0]['tipopagove']." - ".$ve[0]['formapagove']),0,1,'L');
  $this->SetX(4);
  $this->CellFitSpace(60, 3,"EFECTIVO: ".utf8_decode($ve[0]['montopagado']),0,1,'L');
  $this->SetX(4);
  $this->CellFitSpace(60, 3,"VUELTO: ".utf8_decode($ve[0]['montodevuelto']),0,1,'L');
  $this->Ln();
  if($ve[0]['fechavencecredito']== '0000-00-00') { 
  $this->SetX(4);
  $this->CellFitSpace(60, 3,"VENTA ".utf8_decode($ve[0]['statusventa']),0,1,'L');
	...
	$this->SetX(4);
    $this->CellFitSpace(60, 3, DNI." CLIENTE: ".utf8_decode($ve[0]['cedcliente']),0,1,'L');
	$this->SetX(4);
  ...
  $this->SetX(4);
  $this->CellFitSpace(65,2,"**PRECIOS DE PRODUCTOS INCLUYEN IMPUESTOS**",0,1,'C');
...
############################################### FUNCION LISTAR VENTAS GENERALES ######################################################
   ...
   {	
	...
	$this->CellFitSpace(28,8,'SUBTOT',1,0,'C', True);
	$this->CellFitSpace(28,8,'SUBTOT '.IMPUESTO.' 0%',1,0,'C', True);
	$this->CellFitSpace(12,8,IMPUESTO,1,0,'C', True);
	$this->CellFitSpace(22,8,'TOTAL '.IMPUESTO,1,0,'C', True);
...
############################################### FUNCION LISTAR CREDITOS POR CLIENTES ######################################################
	  function TablaCreditosClientes()
   {
	...
	$this->SetFillColor(2,157,116);
	$this->Cell(10,5,'',0,0,'');
	$this->Cell(35,5,DNI.' CLIENTE: ',0,0,'');
...
############################################### FUNCION LISTAR CREDITOS POR FECHAS ######################################################
	  function TablaCreditosFechas()
   {
    ...
    $this->SetFillColor(249, 187, 31); // establece el color del fondo de la celda (en este caso es NARANJA)
	$this->CellFitSpace(8,8,'N°',1,0,'C', True);
	$this->CellFitSpace(20,8,DNI,1,0,'C', True);
...
################################################# BLOQUE N° 5 DE TOTALES ###################################################	
	...
	$this->SetFont('courier','B',9);
    $this->SetXY(143, 258);
    $this->Cell(20, 5, IMPUESTO.' '.$dev[0]["ivad"].'% :', 0 , 0);
    $this->SetXY(167, 258);
	$this->SetFont('courier','',9);
...
############################################### FUNCION LISTAR VENTAS POR FECHAS Y CAJAS DE VENTAS ######################################################
	  ...
	  function TablaServiciosCajas()
   {
	...
	$this->CellFitSpace(25,8,'SUBTOTAL',1,0,'C', True);
	$this->CellFitSpace(22,8,IMPUESTO,1,0,'C', True);
