...
?>
	...
	public function RecuperarPassword()
	{
		...
		$header = 'From: ' . 'FACTURACI�N E INVENTARIOS';
		$msjCorreo = " Nombre: $nombres\n Nuevo Passw: $pass\n Mensaje: Por favor use esta nueva clave de acceso para ingresar al Sistema de Facturaci�n e Inventarios\n";
			...
			if ($num > 0) {
				...
				if ($num == 0) {
					...
					if (isset($_FILES['imagen']['name'])) {
						if ($_FILES['imagen']['name'] != "" && $_FILES['imagen']['size'] >= 1) {
							...
							if ($tipo_archivo == 'image/jpeg' && $tamano_archivo <= 204800) {
								...
								echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
								echo "LA IMAGEN SUPERA EL LIMITE DE PESO O NO ES DEL FORMATO ESPECIFICADO</div>";
								echo "</div>";
								exit;
							}
						} else {
					...
					if ($nombre_archivo != "" && $tipo_archivo != "") {
						if (move_uploaded_file($_FILES['imagen']['tmp_name'], "fotos/" . $nombre_archivo) && rename("fotos/" . $nombre_archivo, "fotos/" . $_POST["cedula"] . ".jpg")) {
							...
							$textImg = "AVISO: No hemos podido cargar la imagen";
						}
					}
					##################  FINALIZA SUBIR FOTO DE USUARIOS ######################################
					...
					echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
					echo "<span class='fa fa-check-square-o'></span> EL USUARIO FUE REGISTRADO EXITOSAMENTE. $textImg</div>";
					echo "</div>";
					exit;
				} else {
					echo "4";
		...
		if ($num > 0) {
			...
			if ($num > 0) {
				...
				if ($num == 0) {
					...
					##################  FINALIZA SUBIR FOTO DE USUARIOS ADMINSTRADOR ######################################


					echo "<div class='alert alert-success'>
		    	<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
      	 	<span class='fa fa-check-square-o'> EL USUARIO FUE REGISTRADO EXITOSAMENTE. $textImg</span>
					<hr>
					 <a href='index'>Logearse</a>
        </div>";
					exit;
				} else {
					echo "4";
	...
	public function RegistrarCompras()
	{
		...
		if (empty($_SESSION["CarritoCompras"])) {
			...
			for ($i = 0; $i < count($compra); $i++) {
				...
				$lote = strip_tags($_POST['lote']);
				if (strip_tags($compra[$i]['vence'] == "")) {
					$vence = "0000-00-00";
				} else {
					$vence = strip_tags(date("Y-m-d", strtotime($compra[$i]['vence'])));
				}
