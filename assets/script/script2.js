...
$(document).ready(function () {
  ...
  $(".current-year").text(
    new Date().getFullYear() +
      " CopyRight. Todos los derechos reservados. Facturacion e Inventarios."
  );
...
function NumberFormat(num, numDec, decSep, thousandSep) {
  ...
  if (Dec == 1) return (sign ? "" : "-") + num;
  else return (sign ? "" : "-") + num + decSep + cents;
}

function EvaluateText(cadena, obj) {
  opc = false;
  if (cadena == "%d") if (event.keyCode > 47 && event.keyCode < 58) opc = true;
  if (cadena == "%f") {
    if (event.keyCode > 47 && event.keyCode < 58) opc = true;
    if (obj.value.search("[.*]") == -1 && obj.value.length != 0)
      if (event.keyCode == 46) opc = true;
  }
...
function VerUsuario(codigo) {
  $("#muestrausuariomodal").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );
...
$(document).ready(function () {
  $(".calculodescuentoc").keyup(function () {
    ...
    Subtotal =
      parseFloat(txtsubtotal) + parseFloat(txtsubtotal2) + parseFloat(txtIva);
    TotalDescuentoGeneral =
      parseFloat(Subtotal.toFixed(2)) * parseFloat(descuento.toFixed(2));
    TotalFactura =
      parseFloat(Subtotal.toFixed(2)) -
      parseFloat(TotalDescuentoGeneral.toFixed(2));
...
function VerDetalleCompra(coddetallecompra) {
  $("#muestradetallecompramodal").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );
  var dataString =
    "BuscaDetallesComprasModal=si&coddetallecompra=" + btoa(coddetallecompra);
  $.ajax({
    type: "GET",
    url: "funciones.php",
    data: dataString,
...
function BuscaDevolucionProductos() {
  $("#muestracomprasdevoluciones").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );
  var codproveedor = $("#codproveedor").val();
...
function VerDetalleVentas(coddetalleventa) {
  $("#muestradetalleventamodal").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );

  var dataString =
    "BuscaDetallesVentasModal=si&coddetalleventa=" + btoa(coddetalleventa);
...
function NuevoAbono(cedcliente, codventa) {
  ...
  var dataString =
    "MuestraFormularioAbonos=si&cedcliente=" +
    btoa(cedcliente) +
    "&codventa=" +
    btoa(codventa);
...
function BuscaCreditosClientesReportes() {
  $("#muestracreditosclientesreportes").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );
  var codcliente = $("#codcliente").val();
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      $("#muestracreditosclientesreportes")
        .append("" + response + "")
        .fadeIn("slow");
      $("#" + parent).remove();
    },
...
function BuscaCreditosFechasReportes() {
  $("#muestracreditosfechasreportes").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );
...
$(document).ready(function () {
  $(".verdetalledevolucion").click(function () {
    $("#muestradetalledevolucionmodal").html(
      '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
    );

    var coddetalledevolucion = btoa($(this).attr("data-id"));
    var dataString =
      "BuscaDetallesDevolucionesModal=si&coddetalledevolucion=" +
...
function VerDetalleServicio(coddetalleservicio) {
  $("#muestradetalleserviciomodal").html(
    '<center><img src="assets/img/loading.gif" width="30" height="30"/></center>'
  );

  var dataString =
    "BuscaDetallesServiciosModal=si&coddetalleservicio=" +
    btoa(coddetalleservicio);
